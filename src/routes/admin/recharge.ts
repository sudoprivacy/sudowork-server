...
rechargeRoutes.post("/recharge/orders/:id/retry", authMiddleware, adminMiddleware, async (c) => {
  const adminUser = (await getAuthUser(c)) as any;
...
rechargeRoutes.post("/recharge/sync", authMiddleware, adminMiddleware, async (c) => {
  const { rechargeService } = await import("../../services/RechargeService.js");
...
rechargeRoutes.post("/recharge/orders/:orderNo/sync", authMiddleware, adminMiddleware, async (c) => {
  const orderNo = c.req.param("orderNo");
  if (!orderNo) {
    return c.json({ success: false, msg: "订单号不能为空" }, 400);
  }
