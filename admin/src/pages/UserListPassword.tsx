...
const UserListPassword: React.FC = () => {
  ...
  const handleCreate = async (values: any) => {
    try {
      const response = await adminApi.createPasswordUser({
        phone: values.phone,
      ...
      const errorMsg =
        error.response?.data?.msg || error.message || "创建失败,请检查网络连接";
  ...
  const handleUpdate = async (values: any) => {
    if (!editingId) return;
    try {
      // role 变更走共用端点 setUserRole
      const cur = users.find((u) => u.id === editingId);
      if (cur && values.role && values.role !== cur.role) {
        try {
      ...
      const response = await adminApi.updatePasswordUser(editingId, updateData);
      if ((response as any).success) {
    ...
    Modal.confirm({
      ...
      content: (
        <div>
          ...
          >
            <p style={{ color: "#d48806", fontWeight: "bold", marginBottom: 8 }}>
    ...
    <div>
      ...
      >
        ...
        >
          ...
          >
            <Input placeholder="请输入用户名称" disabled={!!editingId} />
          </Form.Item>
          <Form.Item label="昵称" name="nickname">
            <Input placeholder="请输入昵称(空则用用户名)" />
          </Form.Item>
          <Form.Item
            label="密码"
            name="password"
      ...
      <Modal
        title={`后台充值 - ${
          users.find((u) => u.id === rechargingUserId)?.nickname ||
          users.find((u) => u.id === rechargingUserId)?.phone
        }`}
                ...
                >
                  ...
                  <div style={{ color: "#86909c", fontSize: 13, marginTop: 4 }}>
                    ...
                    {(() => {
                      ...
                      const currentPoints = user
                        ...
                        : 0;
                      const addedPoints = rechargeAmount * 1000;
                  ...
                  <div style={{ color: "#86909c", fontSize: 13, marginTop: 4 }}>
                    ...
                    {(() => {
                      ...
                      const addedQuota = rechargeAmount * 1000 * 500;
...
export default UserListPassword;
